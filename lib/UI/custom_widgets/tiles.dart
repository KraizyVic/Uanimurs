...
class ErrorMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Image.asset("lib/Database/assets/ms_girl__gundam_heavyarms_custom_ew__48__by_bryanz09_dcz1ct2.png",height: 150,width: 150,),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        TextButton(
                          onPressed: (){
                            if(! state!.isLoggedIn && Supabase.instance.client.auth.currentSession == null){
                              BlocProvider.of<AppCubit>(context).removeFromWatchHistory(watchHistory);
                              Navigator.pop(context);
                            }else{
                              WatchHistoryService().deleteWatchHistory(watchHistory: watchHistory);
                              Navigator.pop(context);
                            }
                          },
                          child: Text("Delete")
...
Widget supabaseWatchListTile({
  required BuildContext context,
  required SupabaseWatchListModel anime,
